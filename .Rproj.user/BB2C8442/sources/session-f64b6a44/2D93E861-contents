...
for (i in 1:10) {
  print(i)
  run_samp(i, N = nrow(sim_pop$X_data))
}
...
run_samp <- function(i, N) {
  ...
  weights1 <- weights_temp$weights1
...
get_weights <- function(dat_B, dat_R, ind_B, ind_R, num_post, pi_R,
  ...
  return(list(weights1 = weights1, wts_post = wts_post))
}
...
var_adjust <- function(res, wts_post, alpha = NULL, eta = NULL, num_reps = 100, 
  ...
  ### CHANGED: CHECK THIS IS CORRECT for scale, rscales, combined.weights
  # Create svrepdesign
